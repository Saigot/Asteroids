...
public class Bosteroid extends Asteroid {
	...
	public void render(GameContainer gc, Graphics g) {
		...
		g.fillOval(x - SIZE * 0.045f, y - SIZE * 0.045f, SIZE * 0.09f,
				SIZE * 0.09f);
	}
	...
	public void Move(Entity e) {
		...
		shape = (Polygon) shape.transform(Transform.createTranslateTransform(
				xv, yv));
	}
	...
	public void Collides(Entity... en) {
		...
		for (Entity e : en) {
			...
			if (e == null || p == null || e == this || e.Cull()
					|| getBounds() == null) {
				continue;
			}
			e.Collides(this);
			if (getBounds().intersects(p) || getBounds().contains(p)) {
				if (e.GetType().equals("Asteroid")) {
					...
					Bullet b = (Bullet) e;
					Asteroid as = new Asteroid(b.x, b.y, b.DoDamage() * 1.4f,
							null, 5);
	...
	public int GetClosestPoint(float x, float y) {
		...
		for (int i = 0; i <= shapept.length - 2; i += 2) {
			double dis = Math.sqrt(Math.pow(shapept[i], 2)
					+ Math.pow(shapept[i + 1], 2));
			if (dis < closest) {
				closest = dis;
				best = i;
			}
	...
	public Shape GetLargestShape(Shape... a) {
		...
		for (Shape s : a) {
			...
			if (result == null
					|| s.getBoundingCircleRadius() > result
