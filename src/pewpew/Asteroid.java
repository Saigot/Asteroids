...
public class Asteroid extends Enemy {
	...
	public Asteroid(float X, float Y, Player p) {
		...
		if (p != null) {
			while (Math.sqrt(((X - x) * (X - x)) + ((Y - y) * (Y - y))) < (200 * p
					.GetScale()) + (size)) {
				X = (float) Math.random() * FORM_WIDTH;
				Y = (float) Math.random() * FORM_HEIGHT;
			}
		...
		for (int i = 0; i <= points * 2 - 1; i += 2) {
			float pX = (float) (size / 2
					* Math.sin(angle * i + (Math.random() * angle)) + Math
					.random() * 20) + Y;
			float pY = (float) (size / 2
	...
	public void Move(Entity e) {
		...
		tick++;
		if (xv == 0 && yv == 0) {
			float speed = (float) (Math.random() * TOP_SPEED)
					+ (TOP_SPEED * 0.1f);
